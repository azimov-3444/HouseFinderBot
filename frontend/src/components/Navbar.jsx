...
import { IoMenu, IoClose, IoHome, IoGrid, IoLogOut, IoSettingsSharp, IoLogIn, IoAddCircle } from 'react-icons/io5';
...
const Navbar = () => {
  ...
  const inactiveStyle = isHeroNav
    ...
    : "text-slate-600 hover:text-primary-600 font-medium pb-1 transition-colors";
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-12 items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <span className={`text-xl font-bold ${isHeroNav ? 'text-white' : 'bg-gradient-to-r from-primary-900 to-primary-700 bg-clip-text text-transparent'}`}>
          ...
          <div className="hidden md:flex items-center gap-4">
            <div className="flex items-center gap-2">
              <select
                ...
                className="h-10 rounded-xl border border-slate-200 bg-white/90 px-3 text-xs font-bold text-slate-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                aria-label="Language"
              >
                {LANGUAGES.map((item) => (
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
                  ...
                  <div className="flex flex-col text-left">
                    ...
                    <span className="text-xs text-slate-500">{user.role === 'admin' ? 'Admin' : t('client')}</span>
                  </div>
                </div>
                <button
              ...
              <Link
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium shadow-md active:scale-95 transition-all ${
                  isHeroNav
                    ...
                    : 'bg-primary-900 text-white hover:bg-primary-800 shadow-primary-900/10 hover:shadow-primary-900/20'
                }`}
              >
            ...
            <button
              ...
              className={`inline-flex items-center justify-center rounded-xl p-2.5 transition-all border ${
                isHeroNav
                  ...
                  : 'border-slate-100 text-slate-500 hover:bg-slate-100/50 hover:text-slate-800'
              }`}
            >
          ...
          <div className="space-y-1 px-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-2 pt-3">
              <select
                value={language}
                onChange={(e) => setLanguage(e.target.value)}
                className="w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm font-bold text-slate-700"
                aria-label="Language"
              >
                {LANGUAGES.map((item) => (
                  <option key={item.code} value={item.code}>{item.label}</option>
